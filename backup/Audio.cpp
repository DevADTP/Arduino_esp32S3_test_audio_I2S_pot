...
void Audio::processLocalFile() {
	...
	{
	audiofile.read(encrypredData, nrOfDecryptedBytes);

    // Déchiffrer le bloc
    mbedtls_aes_crypt_ecb(&aes, MBEDTLS_AES_DECRYPT, encrypredData, decrypredData);
	
	//buffer transfert ram to audio
	memcpy(InBuff.getWritePtr(), decrypredData, nrOfDecryptedBytes); 
	bytesAddedToBuffer += nrOfDecryptedBytes;
	}

	//int32_t bytesAddedToBuffer = nrOfDecryptedBytes;
	*/
	// ----------- FIN METHODE 1 ------------
	...
	if(availableBytes > 2048)
	{
		...
		{	
		audiofile.read(encrypredData, 16);
		
		mbedtls_aes_crypt_ecb(&aes, MBEDTLS_AES_DECRYPT, encrypredData, decrypredData);
		
		for (int dd=0;dd<16;dd++)
		{
			decrypredDataBig[cc*16+dd]=decrypredData[dd];
		}
			
		}
		
		//buffer transfert ram to audio 1024bytes
		memcpy(InBuff.getWritePtr(), decrypredDataBig, 2048); 
		bytesAddedToBuffer = 2048;
	}
	else if(availableBytes >= 64)
	{
		...
		{
			decrypredDataBig[ee*16+ff]=decrypredData[ff];
		}
			
		}
		
		//buffer transfert ram to audio 1024bytes
		memcpy(InBuff.getWritePtr(), decrypredDataBig, 64); 
		bytesAddedToBuffer = 64;
	}
	else
	{
